...
inline std::tuple< std::vector<double>, std::vector<double> > Load_XY_CSV_Data( const std::string & data_to_parse, const char* delimiter = "," )
{
	...
	// Vector of rows
inline std::vector< std::vector<std::string> > Load_CSV_Data( const std::string & data_to_parse, const char* delimiter = "," )
{
	using namespace std;
	using namespace boost;
	...
	output.reserve( split_by_line.size() );
	for( const auto &[ line_number, one_line ] : split_by_line % fn::transform( fn::get::enumerated{} ) )
	{
		if( one_line.size() == 0 )
			continue; // Ignore blank lines
		...
		output.emplace_back( std::move( split_by_commas ) );
	}
